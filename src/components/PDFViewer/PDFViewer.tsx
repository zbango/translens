    ...
    <div className="relative w-full h-full">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3 text-sm text-slate-300">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (Number.isFinite(next))
                setPageNumber(Math.min(Math.max(1, next), numPages || 1));
            }}
          />
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            onClick={() => {
              ...
              if (containerWidth && pageWidth) {
                ...
                const nextScale = Math.max(
                  0.5,
                  Math.min(3, (containerWidth - gutter) / pageWidth)
                );
                setScale(nextScale);
              }
            }}
          ...
          >
            {showTextLayer
              ? t("viewer.hideTextLayer")
              : t("viewer.showTextLayer")}
          </button>
          <button
      ...
      >
        <Document
          ...
          loading={<Loader />}
          error={
            <div className="text-red-400 text-sm">{t("viewer.loadFailed")}</div>
          }
        >
          {continuous ? (
